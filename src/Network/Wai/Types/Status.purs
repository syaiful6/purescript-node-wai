...
data Status = Status Int String

instance eqStatus :: Eq Status where
  eq (Status c1 _) (Status c2 _) = c1 == c2

instance ordStatus :: Ord Status where
  compare (Status c1 _) (Status c2 _) = compare c1 c2
...
statusIsInformational (Status code _) = code >= 100 && code < 200

-- | Successful class
statusIsSuccessful :: Status -> Boolean
statusIsSuccessful (Status code _) = code >= 200 && code < 300

-- | Redirection class
statusIsRedirection :: Status -> Boolean
statusIsRedirection (Status code _) = code >= 300 && code < 400

-- | Client Error class
statusIsClientError :: Status -> Boolean
statusIsClientError (Status code _) = code >= 400 && code < 500

-- | Server Error class
statusIsServerError :: Status -> Boolean
statusIsServerError (Status code _) = code >= 500 && code < 600
